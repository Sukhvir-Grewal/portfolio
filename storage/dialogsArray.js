const dialogsArray = {
    dashboard: {
        script1: [
            "Hey there! I'm Sukhvir, just like the domain says. Great to meet you, mysterious visitor!",
            "Step right up for a peek into my world!",
            "Every click is a new adventure. Ready to start yours?",
            "Peek behind the curtain and see what I've been up to. Spoiler alert: It's exciting stuff!",
        ],
        images: [
            "/images/myImages/dashboard-1.png",
            "/images/myImages/dashboard-2.png",
            "/images/myImages/dashboard-3.png",
            "/images/myImages/dashboard-4.png",
        ],
    },
    about: {
        script1: [
            "Apologies, but around here, it's always raining cats and dogs!",
            // "Curious about the person behind the screen? You've come to the right place for stories and more!",
            "By the way, dive in here to uncover all my secrets!",
        ],
        finish: "And there you have it, the complete 411 on me!",
        lastVisit:
            "Look at us, strangers no more – we're practically pals now!",
        images: [
            "/images/myImages/about-1.png",
            "/images/myImages/about-2.png",
        ],
        lastVisitImage:"/images/myImages/about-01.png"
    },
    skill: {
        script1: [
            "That's all the wisdom I've got in my pocket for now...",
            "However, like a magician, I'm always adding new tricks to my repertoire.",
            "Remember, every master was once a beginner.",
        ],
        lastVisit: "So, that's my knowledge shared – you're in the loop now!",
        howToOperate: "Give a click and discover just how much I know!",
        ohh: "Interesting!!",
        images: [
            "/images/myImages/skill-1.png",
            "/images/myImages/skill-2.png",
            "/images/myImages/skill-3.png",
        ],
        howToOperateImage: "/images/myImages/skill-01.png",
        ohhImage: "/images/myImages/skill-02.png",
        lastVisitImage:"/images/myImages/skill-03.png"
    },
    //Images done
    project: {
        script1: [
            "Check out these handmade masterpieces, no AI magic, just pure brilliance!",
            "Head over to our project website for a real eye-opener – it's horizon time!",
            "My projects are my dreams woven into reality.",
            "Keen to read what I've written? Prepare for a rollercoaster of words and wit!",
        ],
        lastVisit:
            "These projects may not be the top of the line, but they're great examples of my abilities.",
        images: [
            "/images/myImages/project-1.png",
            "/images/myImages/project-01.png",
            "/images/myImages/project-2.png",
            "/images/myImages/project-3.png",
        ],
        lastVisitImage:"/images/myImages/project-02.png"
        
    },
    contact: {
        script1: [
            "Now you've got the scoop on me, it's your turn – drop me a line!",
            "Got questions, ideas, or just want to chat? I'm all ears – let's connect!",
        ],
        lastVisit:
        "Already dropped me a line? That's awesome – and if not, don't be shy to reach out!",
        loading: "Sending...",
        sent: "Got your message, and I'll reply within a day. My lonely heart can't wait much longer!",
        images: [
            "/images/myImages/contact-1.png",
            "/images/myImages/contact-2.png",
        ],
        loadingImage: "/images/myImages/contact-01.png",
        sentImage: "/images/myImages/contact-02.png",
        lastVisitImage:"/images/myImages/contact-03.png",
    },
};

export default dialogsArray;
